...
barWidths = linspace(0.6667, 0.5, 4);
colors = {'#0085c3', '#14d4f4', '#f2af00', '#b7295a', '#00205b', '#009f4d', '#84bd00', '#efdf00', '#e4002b', '#a51890'};

ave_diameter = [15.8347775164174, 16.3916376276153, 19.0758298864265, 34.3026879560674].*2;

% Main loop to process each temperature point
for iTemperature = 2:2 %length(temperatures)
    ...
    for iBoxNum = 4:4 %1:length(localRegions)
        for iTimePoint = 1:length(fileTimes)
            ...
            x = statisticsTable.grainSize .* ave_diameter(iTimePoint);
            y = statisticsTable.areaFraction .* 100;
