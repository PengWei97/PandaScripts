% exp13_get_ave_rho_with_depth.m
%
% Purpose:
% This script processes EBSD data to calculate the average dislocation density with depth for different temperatures.
% It generates average dislocation density profiles for specified time points and saves the results as CSV files.
%
% Usage:
% Simply run this script to generate the average dislocation density profiles for each temperature and time point.
...
for iTemperature = 1:length(temperatures)
    ...
    for iTimePoint = 1:length(fileTimes)
        ...
        hold on;
        plot(ebsdGrid.boundary, 'linewidth', 0.8);
        hold off;

        % Calculate average dislocation density with depth
        [xmin, xmax, ymin, ymax] = ebsdGrid.extend;
        aveRhoTable = table('Size', [length(ebsdGrid.id(:,1)), 2], 'VariableTypes', {'double', 'double'}, 'VariableNames', {'Depth', 'AveRho'});
        for iAveRho = 1:length(ebsdGrid.id(:,1))
            rhoSequence = rho(ebsdGrid.id(iAveRho,:));
            rhoSequenceNotNAN = rhoSequence(~isnan(rhoSequence));
            aveRhoTable.AveRho(iAveRho) = mean(rhoSequenceNotNAN);
        end
        aveRhoTable.Depth = linspace(0, ymax - ymin, length(ebsdGrid.id(:,1)))';

        % Save average dislocation density with depth
        outputFileAveRho = fullfile(outputDir, sprintf('Ti%ddu_ave_rho_%s.csv', temperatures(iTemperature), fileTimes{iTimePoint}));
        if ~exist(fileparts(outputFileAveRho), 'dir')
            mkdir(fileparts(outputFileAveRho));
        end
        writetable(aveRhoTable, outputFileAveRho);
    end
end
...
function [inputDir, fileTimes, timePoints, outputDir] = setDirectoriesAndTimes(iTemperature)
    ...
    if iTemperature == 1
        ...
        outputDir = 'D:\Github\PandaData\p21_GMSTi_AGG_2023\exp_Ti550du\csv2\csv_ave_rho_with_depth_v2\';
        ...
        outputDir = 'D:\Github\PandaData\p21_GMSTi_AGG_2023\exp_Ti700du\csv2\csv_ave_rho_with_depth_v2\';
